...
export default function UuidGenerator() {
  ...
  useEffect(() => {
    const worker = new Worker(new URL('./uuidWorker.js', import.meta.url), {
      type: 'module',
    });
    ...
    worker.onmessage = (event) => {
      ...
      if (!response?.success) {
        setStatus({
          type: 'error',
          message: response?.error?.message || 'UUID generation failed.',
        });
        return;
      }
      ...
      setStatus({
        type: 'success',
        message: `Generated ${result.length} UUID${result.length === 1 ? '' : 's'}.`,
      });
    };
  ...
  const copyAll = useCallback(async () => {
    ...
    try {
      ...
      setStatus({
        type: 'success',
        message: `Copied ${items.length} UUID${items.length === 1 ? '' : 's'} to clipboard.`,
      });
    } catch {
      setStatus({
        type: 'error',
